...
if [ -z "$1" ]; then
	for CATEGORY in $(find -L tests -mindepth 1 -maxdepth 1 -type d | sort -V); do
		for T in $(find -L "$CATEGORY" -mindepth 1 -maxdepth 1 -type d | sort -V); do
			run_test "$T"
		done
	done
elif [ -z "$2" ]; then
	for T in $(find -L "tests/$1" -mindepth 1 -maxdepth 1 -type d | sort -V); do
		run_test "$T"
	done
else
	run_test "tests/$1/$2"
