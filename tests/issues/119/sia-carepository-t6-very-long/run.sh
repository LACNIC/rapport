...
# Issue #119 -- SIA caRepository: very long URI (potential buffer overflow)
#
...
printf '\n' >> "$FILE_RD"
printf '[node: ca.cer]\n' >> "$FILE_RD"
printf 'obj.tbsCertificate.extensions.sia.extnValue.0.accessLocation.value = rsync://localhost:8873/rpki/%s/' \
    "$TEST" >> "$FILE_RD"
