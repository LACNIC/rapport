...
# IPv4 δ — chgmaxln: W(maxlen=24) + A(maxlen=16) for AS 65552-65561
p=16; while [ "$p" -le 25 ]; do
	as=$((65536+p))
	set -- "$@" \
		"ipv4-prefix    version 2 zero1 0 length 20 flags 1 plen 16 maxlen 16 zero2 0 prefix 10.$p.0.0 as $as" \
		"ipv4-prefix    version 2 zero1 0 length 20 flags 0 plen 16 maxlen 24 zero2 0 prefix 10.$p.0.0 as $as"
	p=$((p+1)); done

# IPv4 δ — interleaved: alternating W(rd1-only,even) + A(rd2-only,odd)
ep=26; op=27
while [ "$ep" -le 84 ]; do
	set -- "$@" \
		"ipv4-prefix    version 2 zero1 0 length 20 flags 0 plen 16 maxlen 16 zero2 0 prefix 10.$ep.0.0 as $((65536+ep))" \
		"ipv4-prefix    version 2 zero1 0 length 20 flags 1 plen 16 maxlen 16 zero2 0 prefix 10.$op.0.0 as $((65536+op))"
	ep=$((ep+2)); op=$((op+2)); done
...
k=0; while [ "$k" -le 9 ]; do
	...
	set -- "$@" \
		"aspa-pdu       version 2 flags 1 zero 0 length 20 customer $_cust providers \[ $((64900+k)) $((64901+k)) \]"
	k=$((k+1)); done
...
while [ "$ep" -le 84 ]; do
	set -- "$@" \
		"aspa-pdu       version 2 flags 0 zero 0 length 12 customer $((0x88000000+ep*65536))" \
		"aspa-pdu       version 2 flags 1 zero 0 length 16 customer $((0x88000000+op*65536)) providers \[ 65300 \]"
	ep=$((ep+2)); op=$((op+2)); done
