...
# Rewrite the snapshot's own serial (2 -> 99) so it no longer matches the
# serial the notification announces (2).
sed -i 's/serial="2">/serial="99">/' "$SNAPSHOT"

# Recompute the snapshot's SHA-256 and re-align it in the notification so hash
# verification passes and the serial cross-check is the operative one.
