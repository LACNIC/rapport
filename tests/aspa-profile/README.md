...
> main paragraph it relates to in draft-26 (e.g. `#section-2-1` → `2-1`).
> When more than one test relates to the same paragraph, a sub-index (`-a`, `-b`,
> `-c`, …) is appended (`2-1-a`, `2-1-b`, …).
...
**Related sections:** section 2 (the ASPA content-type is
`id-ct-ASPA` with the numerical value `1.2.840.113549.1.9.16.1.49`; this OID MUST
appear both in the `eContentType` of `encapContentInfo` and in the `content-type`
signed attribute of `signerInfo`)
...
**Description:**
This test checks that the validator accepts an ASPA object whose `content-type`
signed attribute within the `signerInfo` structure
(`signerInfos.0.signedAttrs.content-type`) correctly carries the OID
`1.2.840.113549.1.9.16.1.49` (`id-ct-ASPA`), matching the `eContentType` in the
`encapContentInfo` structure, allowing the object to proceed through subsequent
validation steps.

**Related sections:** section 2 (the OID MUST appear both in the
`eContentType` of `encapContentInfo` and in the `content-type` signed attribute of
`signerInfo`)
...
**Description:**
This test checks that the validator rejects an ASPA object whose `content-type`
signed attribute within the `signerInfo` structure
(`signerInfos.0.signedAttrs.content-type`) does not carry the OID
`1.2.840.113549.1.9.16.1.49` (`id-ct-ASPA`) — for example, when it differs from
the OID present in the `eContentType` of the `encapContentInfo` structure —
treating it as a structurally non-compliant signed object that cannot be used for
...
**Related sections:** section 3 (the ASN.1 module defines
`PAS ::= INTEGER (0..4294967295)`; the value `1` is a valid `PAS`)
...
**Related sections:** section 3 (the ASN.1 module defines
`PAS ::= INTEGER (0..4294967295)`; the value `4294967294` is within the valid range)
...
by the ASN.1 type `PAS ::= INTEGER (0..4294967295)`, confirming that the boundary
value is handled correctly and does not cause an off-by-one rejection.

**Related sections:** section 3 (the ASN.1 module defines
`PAS ::= INTEGER (0..4294967295)`; the value `4294967295` is the maximum valid `PAS`)
...
`4294967295` defined by the ASN.1 type `PAS ::= INTEGER (0..4294967295)`, making
the encoded value structurally out of the permitted range.

**Related sections:** section 3 (the ASN.1 module defines
`PAS ::= INTEGER (0..4294967295)`; the value `4294967296` is one beyond the upper
bound and is therefore not a valid encoding)
...
`ASProviderAttestation` that complies with this specification MUST be 1 and MUST be
explicitly encoded)
...
**Related sections:** section 3.1 (the version number of the
`ASProviderAttestation` MUST be 1 and MUST be explicitly encoded)
...
**Related sections:** section 3.1 (the version number MUST be 1
and MUST be explicitly encoded; the ASN.1 definition declares
`version [0] INTEGER DEFAULT 0`, but a compliant object must override the default
with an explicit `1`)
...
field is set to `1`, the minimum value permitted by the ASN.1 type
`CAS ::= INTEGER (1..4294967295)`, confirming that the lower boundary of the
Customer AS range is handled correctly and does not cause an off-by-one rejection.

**Related sections:** section 3.2 (`customerASID` is a positive
integer naming the Customer AS); ASN.1 type `CAS ::= INTEGER (1..4294967295)` in the
Section 3 defines `1` as the minimum valid value
...
**Description:**
This test checks that the validator accepts an ASPA object whose `customerASID`
field is set to `2`, an ordinary value well within the valid range of the ASN.1
type `CAS ::= INTEGER (1..4294967295)`, confirming that a typical small Customer AS
identifier is processed without rejection.

**Related sections:** section 3.2 (`customerASID` is a positive
integer naming the Customer AS); ASN.1 type `CAS ::= INTEGER (1..4294967295)` in the
Section 3 includes `2` in the valid range
...
**Description:**
This test checks that the validator accepts an ASPA object whose `customerASID`
field is set to `4294967294`, which is one below the maximum allowed value and is
within the valid range of the ASN.1 type `CAS ::= INTEGER (1..4294967295)`,
confirming that near-maximum Customer AS identifiers are processed correctly.

**Related sections:** section 3.2 (`customerASID` is a positive
integer naming the Customer AS); ASN.1 type `CAS ::= INTEGER (1..4294967295)` in the
Section 3 includes `4294967294` in the valid range
...
**Description:**
This test checks that the validator accepts an ASPA object whose `customerASID`
field is set to `4294967295`, the maximum value defined by the ASN.1 type
`CAS ::= INTEGER (1..4294967295)`, confirming that the upper boundary of the
Customer AS range is handled correctly and does not cause an off-by-one rejection.

**Related sections:** section 3.2 (`customerASID` is a positive
integer naming the Customer AS); ASN.1 type `CAS ::= INTEGER (1..4294967295)` in the
Section 3 defines `4294967295` as the maximum valid value
...
field is set to `4294967296`, which is one beyond the maximum value of
`4294967295` defined by the ASN.1 type `CAS ::= INTEGER (1..4294967295)`, making
the encoded value structurally out of the permitted range.

**Related sections:** section 3.2 (`customerASID` is a positive
integer naming the Customer AS); ASN.1 type `CAS ::= INTEGER (1..4294967295)` in the
Section 3 excludes `4294967296`, one beyond the upper bound
...
**Related sections:** section 3.2 (`customerASID` is a positive
integer naming the Customer AS); ASN.1 type `CAS ::= INTEGER (1..4294967295)` in the
Section 3 excludes `0` from the valid range
...
**Related sections:** section 3.3 (the CustomerASID value
MUST NOT appear in any `PAS` in the `providers` field)
...
**Related sections:** section 3.3 (the elements of
`providers` MUST be ordered in ascending numerical order)
...
**Related sections:** section 3.3 (each value of `PAS` MUST be unique with respect 
to the other elements of `providers`)
...
**Related sections:** section 3.3 (a `PAS` value of 0 can
only be encoded in the `providers` field as a single-item list; a list containing
only AS 0 satisfies this constraint)
...
**Related sections:** section 3.3 (a `PAS` value of 0 can
only be encoded in the `providers` field as a single-item list; an element for AS 0
MUST NOT appear alongside any other elements)
...
**Related sections:** section 4 (the relying party MUST perform
all the validation checks specified in RFC 6488; RFC 6488 requires the EE
certificate in the CMS structure to be valid under the RPKI certification path,
including signature verification by the issuing CA)
...
**Related sections:** section 4 (the relying party MUST perform
all the validation checks specified in RFC 6488; RFC 6488 requires the CMS
signature in the `signerInfo` structure over the encapsulated content to verify
against the EE certificate's public key)
...
**Related sections:** section 4 (the AS Identifier Delegation
Extension MUST be present in the EE certificate, and the Customer ASID in the ASPA
eContent MUST match the ASId specified by that extension)
...
**Related sections:** section 4 (the relying party MUST perform
all the validation checks specified in RFC 6488, which includes verifying that the
EE certificate's resource extensions are subordinate to those of the issuing CA
certificate, as required by the RPKI certification hierarchy)
...
**Related sections:** section 4 (the AS Identifier Delegation
Extension MUST contain exactly one `id` element and MUST NOT contain any `inherit`
elements — section 3.2.3.3 of RFC 3779 — or `range` elements)
...
**Related sections:** section 4, ¶ `section-4-2.2.1` (the AS Identifier Delegation
Extension MUST contain exactly one `id` element — section 3.2.3.6 of RFC 3779 — and
MUST NOT contain any `range` elements — section 3.2.3.7 of RFC 3779 — nor any
`inherit` elements)
...
**Related sections:** section 4 (the IP Address Delegation
Extension of RFC 3779 MUST be absent from the EE certificate)
...
**Related sections:** section 5.1 (for any given Customer AS,
only a single ASPA object SHOULD be maintained containing all providers, to avoid
race conditions during ASPA updates; the CA software that maintains ASPA records
SHOULD support enforcement of this recommendation — note that this guidance targets
the CA and publication software, not the Relying Party validator; the draft does not
specify what a validator MUST or SHOULD do upon encountering multiple valid ASPA
objects for the same `customerASID`)
...
**Related sections:** section 5.4 (an upper bound value between
4,000 and 10,000 Provider ASes is suggested; a list of 3,999 entries is below this
range and should not trigger threshold-based rejection)
...
**Related sections:** section 5.4 (an upper bound value between
4,000 and 10,000 Provider ASes is suggested; the value 4,000 sits at the lower end
of the suggested range and SHOULD NOT trigger a threshold-based rejection)
...
**Related sections:** section 5.4 (Relying Party implementations
are RECOMMENDED to impose an upper bound between 4,000 and 10,000 Provider ASes for
a given Customer AS; if the threshold is exceeded, implementations SHOULD treat all
related ASPA objects as invalid — not emitting a partial list — and SHOULD log an
error indicating the Customer AS for which the threshold was exceeded)
...
**Related sections:** section 5.4 (Relying Party implementations
are RECOMMENDED to impose an upper bound and SHOULD treat all related ASPA objects
as invalid if the threshold is exceeded); also (while the ASN.1
profile imposes no limit on the number of Provider ASes, consideration must be given
to limitations in validators and elsewhere in the RPKI ecosystem, e.g. RPKI-to-Router
PDU limits and local maximum file-size restrictions)
